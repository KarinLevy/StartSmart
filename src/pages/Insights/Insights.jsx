...
import { formatDuration, formatGap, formatDate } from '../../utils/dateFormat';
...
const ScoreMeter = ({ score, label }) => {
  ...
  return (
    <svg width="160" height="160" aria-label={`${label} ${score}%`} role="img" style={{ flexShrink: 0 }}>
      <circle cx="80" cy="80" r={r} fill="none" stroke="var(--color-surface-container)" strokeWidth="10" />
      <circle
        cx="80" cy="80" r={r}
        fill="none" stroke={color} strokeWidth="10"
        strokeDasharray={circ} strokeDashoffset={offset}
        strokeLinecap="round"
        style={{ transform: 'rotate(-90deg)', transformOrigin: '80px 80px', transition: 'stroke-dashoffset 0.8s ease' }}
      />
      <text x="80" y="75" textAnchor="middle" fill="var(--color-on-surface)" fontSize="26" fontWeight="800">{score}%</text>
      <text x="80" y="95" textAnchor="middle" fill="var(--color-on-surface-variant)" fontSize="11" fontWeight="600"
        textLength="116" lengthAdjust="spacingAndGlyphs">{label}</text>
    </svg>
  );
};
...
const TrendItem = ({ icon, text, badge = null, positive = null }) => (
  <div className="ins-trend-item">
    ...
    {badge && (
      <span className={`ins-trend-badge${positive === true ? ' good' : positive === false ? ' bad' : ''}`} dir="ltr">
        {badge}
      </span>
    )}
  </div>
...
const Insights = () => {
  ...
  if (done.length > 0) {
    ...
    if (pctOverdue <= 20) TRENDS.push({ icon: 'task_alt', text: t('insights.trendOnTime', { pct: 100 - pctOverdue }), badge: `${100 - pctOverdue}%`, positive: true });
    else TRENDS.push({ icon: 'warning', text: t('insights.trendOver', { pct: pctOverdue }), badge: `${pctOverdue}%`, positive: false });
  }
  if (avgGap !== null) {
    if (avgGap > 10)  TRENDS.push({ icon: 'more_time', text: t('insights.trendAvgOver', { avg: avgGap }), badge: fmtGap_(avgGap), positive: false });
    if (avgGap < -10) TRENDS.push({ icon: 'fast_forward', text: t('insights.trendAvgFast', { avg: Math.abs(avgGap) }), badge: fmtGap_(avgGap), positive: null });
    if (Math.abs(avgGap) <= 10) TRENDS.push({ icon: 'adjust', text: t('insights.trendAvgAccurate', { avg: avgGap }), badge: fmtMin(Math.abs(avgGap)), positive: true });
  }
    ...
    <div className="insights-layout">
      ...
      <main id="main-content" className="insights-main">
        ...
        <div className="ins-header">
          <div>
            <Link to="/statistics" className="btn btn-secondary ins-back">
              <span className="material-symbols-outlined flip-rtl" style={{ fontSize: '1rem' }}>arrow_back</span>
          ...
          <div className="ins-grid">
            ...
            <div className="ins-card ins-card-score">
              ...
              <div className="ins-score-row">
                <ScoreMeter score={score} label={t('insights.scoreLabel')} />
              ...
              <div className="ins-bar-list" role="list" aria-label="Gap per task">
                {done.map((tk) => (
                  <div key={tk.id} className="ins-bar-row" role="listitem">
                    ...
                    <span className={`ins-bar-val${(tk.gap || 0) > 0 ? ' over' : ' under'}`} dir="ltr">
                      {fmtGap_(tk.gap || 0)}
                    </span>
                  </div>
                ))}
